...
class Outsourcing(models.Model):
    ...
    allow_timesheets = fields.Boolean("Timesheets", default=True, help="Enable timesheeting on the outsourcing.")
    ...
    def _check_allow_timesheet(self):
        for outsourcing in self:
            if outsourcing.allow_timesheets and not outsourcing.analytic_account_id:
                raise ValidationError(_('To allow timesheet, your outsourcing %s should have an analytic account set.' % (outsourcing.name,)))

    @api.model
    def name_create(self, name):
        """ Create a outsourcing with name_create should generate analytic account creation """
        values = {
            'name': name,
    ...
    def create(self, values):
        """ Create an analytic account if outsourcing allow timesheet and don't provide one
        ...
        return super(Outsourcing, self).create(values)

    def write(self, values):
        # create the AA for outsourcing still allowing timesheet
        if values.get('allow_timesheets'):
            for outsourcing in self:
                if not outsourcing.analytic_account_id and not values.get('analytic_account_id'):
                    outsourcing._create_analytic_account()
        result = super(Outsourcing, self).write(values)
        return result
...
class Task(models.Model):
    ...
    analytic_account_active = fields.Boolean("Analytic Account", related='outsourcing_id.analytic_account_id.active', readonly=True)
    allow_timesheets = fields.Boolean("Allow timesheets", related='outsourcing_id.allow_timesheets', help="Timesheets can be logged on this task.", readonly=True)
    ...
    def write(self, values):
        # a timesheet must have an analytic account (and a outsourcing)
        if 'outsourcing_id' in values and self and not values.get('outsourcing_id'):
                raise UserError(_('This task must be part of a outsourcing because there are some timesheets linked to it.'))
